...
| **INV-07** | `unauthorized_adapter_call == 0` (§29.6, §26.1) | `AdapterRegistry` allowlist of (adapter, target, selector, pool, token); no free-form call path exists in the contract | `forge` `testNoArbitraryCallSurfaceExists`, `testUnlistedTargetReverts`, `testUnlistedSelectorReverts`, `testNoArbitraryCallSurfaceExists` (bytecode scan for `CALL` outside adapters) | counter + on-chain event | Emergency pause | G-SOL-1 |
...
| ID | Invariant | Implementation | Test | Monitor | Failure response | Gate |
|---|---|---|---|---|---|---|
| **INV-11** | No global mutable state is shared truth between search workers; workers receive immutable snapshots or versioned read handles (§5.3) | `Versioned<T>` over `ArcSwapOption`; `#![forbid]` lint + a CI grep that fails on `Mutex<Option<Arc<` and on `&mut` state crossing a worker boundary | `state::a_reader_keeps_its_snapshot_across_a_write` (compile-fail); CI `scripts/ci/no_shared_mutable_state.sh` | — | Build fails | G-STATE-1 |
| **INV-12** | A feed gap is never silently converted to "probably unchanged" (§5.6) | `FeedIntegrity` tracks `expected_sequence`; a gap sets `ReconstructionStatus::Unsafe` on the affected branch | `state::a_sequence_gap_marks_the_branch_unsafe`; `state::duplicates_and_out_of_order_are_counted_separately_from_gaps` | `gap_count`, `out_of_order_count`, `duplicate_count`, `recovery_time_ms` | Mark UNSAFE → stop tickets → backfill → verify → resume | G-STATE-1 |
| **INV-13** | Two disagreeing feeds are resolved by parentage/sequence/reconciliation/rebuild — **never** by majority vote (§5.6) | `FeedArbiter::resolve` has no vote path; it returns `Rebuild` when parentage is ambiguous | `state::contradicting_feeds_are_never_resolved_by_majority_vote` | `state_fingerprint_mismatch` | Full state rebuild | G-STATE-1 |
| **INV-14** | Speculative branches that diverge from the final block are rolled back and impacted candidates re-evaluated (§5.4) | `SpeculativeStateTree::commit_or_rollback` on each confirmed block | `state::a_divergent_branch_is_rolled_back_and_names_its_candidates`; `state::a_ws_gap_invalidates_every_local_snapshot` | `preconf_to_final_divergence_rate`, `state_branch_reorg_count`, `rollback_cost_ms`, `candidate_invalidated_by_divergence` | Reduce reliance on preconf state; raise margin | G-STATE-2 |
...
| ID | Invariant | Implementation | Test | Monitor | Failure response | Gate |
|---|---|---|---|---|---|---|
| **INV-16** | No router quote is authoritative; the exact integer state machine is (§9, §10.3) | `ExactPricingEngine::quote_exact` is the only pricing input to sizing; quoter calls exist only inside `cl_parity_gate`/validation | `scripts/ci/no_quoter_in_econ.sh` — **not yet written**, recorded 2026-09-24 | — | Build fails | G-PRICE-1 |
...
| **INV-18** | The final size is an exact integer/wei candidate verified by exact AMM evaluation; a continuous optimum is never an execution dependency (§14.3) | `sizing::refine_discrete` is mandatory and returns `DiscreteSize`, the only type `Candidate::input_amount` accepts | `types::discrete_size_requires_a_refinement_witness` (type-level, Phase 0 ✅); `econ::discrete_refinement_is_never_worse_than_the_continuous_optimum` (property, Phase 3) | `continuous_size` vs `final_discrete_size` | — | G-ECON-1 |
...
| **INV-37** | Gas limit is the smallest value `≥ G_safe = Q_gas(simulated) + headroom` that remains valid and fits the earliest economically valuable window; if none exists the ticket is rejected **before signing** (§24.7) | `gas::choose_limit` returns `Option<GasLimit>`; `None` closes the ticket with `EarliestFlashblockTooLate` | `chain::a_smaller_gas_limit_can_buy_an_earlier_window`; `chain::no_safe_gas_limit_rejects_before_signing` | `earliest_eligible_flashblock` vs `actual_flashblock_index` | Reject pre-sign | G-BASE-1 |
...
3. ~~`forge test --no-match-path 'test/Deploy*'` is **52/0, deterministically**, verified over 5 consecutive runs. The 15 `test/Deploy*.t.sol` tests are **quarantined** under B-13 and excluded from this gate until Phase 5 Task 5.7.~~ **Quarantine lifted 2026-09-24 by Task 5.7.** The criterion is now the one originally intended: `forge test`, whole suite, no exclusion, deterministic and green — measured 10/10 runs at 107/0. Note the amendment history: the original wording was "fully green", written before the suite was known to be *flaky* rather than merely red; determinism was and is the real requirement, since a gate that passes 5 runs in 8 certifies nothing.
...
- [x] **Step 2: Run and observe the failures** — both fail today; §3.4 B-13 and `docs/apex/BASELINE.md` carry the measured evidence and the list of approaches already tried and reverted, which must not be retried.
- [x] **Step 3: Implement** a `resolveConfig(DeployConfig explicit)` seam on `Deploy` so the environment is read in exactly one place at the top and everything below is pure. Tests pass a struct; production passes the env-derived one. **Done, and one step further than the sketch:** `runWith(DeployConfig)` was needed as well. `resolveConfig` alone fixes the two *validation* tests, but `DeployOwnership.t.sol` deploys — it calls `run()`, which reads the environment — so the seam had to reach the deploy itself. `run()` is now `runWith(configFromEnv(salt))` and the ownership tests pass an explicit struct. `_isBalancerRequired`/`_isAaveRequired` were folded into `configFromEnv` and deleted; `_validateResolvedIntegrations` now takes the struct.
- [x] **Step 4: Run and observe** the full `forge test` deterministic and green over 10 consecutive runs — the acceptance bar Phase 0 could not meet. **Measured 2026-09-24: 10/10 runs, 107 passed / 0 failed / 0 skipped across 17 suites, no `--no-match-path`.** The suite was 67 tests at the Phase 0 freeze; the other 40 are Phases 1–5.
- [x] **Step 5: Commit**, and lift the B-13 quarantine from the Phase 0 gate wording.

**Two deviations from the Step 1 sketch, both deliberate:**

1. **`testDeployTestsDoNotWriteProcessEnv` is a CI gate, not a test.** The sketch has it call `vm.ffi`. Enabling `ffi` in `foundry.toml` is global — it would hand *every* test in the repository arbitrary shell execution, in a repository whose `.env` holds a plaintext private key, to assert a property about source text that no test needs to observe at runtime. `scripts/ci/no_shared_env_keys.sh` asserts the same property from outside the VM and costs nothing.

2. **The hermetic test must not perturb `CHAIN`.** The sketch's `testResolvedConfigIsIndependentOfAmbientEnv` writes `CHAIN=optimism` then `CHAIN=eth`. Writing it is the disease: `CHAIN` has a *reader* — `DeployEnvPrefixTest::testResolveEnvPrefixMapsOptimismToOpt`, whose subject is prefix resolution — and `vm.setEnv` is process-global, so the sketch as written would have restored B-13 across files while claiming to close it. The test perturbs `REQUIRE_BALANCER` and `UNIV3_ROUTER` instead: production keys that `configFromEnv` reads and that no test reads. This is the rule the gate encodes.

**What the gate checks, and why that is sufficient.** `scripts/ci/no_shared_env_keys.sh` fails if any `vm.setEnv` key under `test/` is written by more than one file, *and* fails if any file other than `script/Deploy.s.sol` calls `vm.env*`. The second half is what makes the first half enough: with a single reader in the tree, a key written by one file has no cross-file race to lose. If a second reader ever appears the gate goes red and says the reasoning has gone stale, rather than silently under-checking. Both halves mutation-tested (a duplicate `CHAIN` write in a second file; a `vm.envOr` in a second file) — each caught.
...
### Task 8.0 — Audit against acceptance criterion 5 (not in the original plan)

Criterion 5 is "all 46 invariants have passing named tests; `invariant_coverage.sh` is green with no allowances". Checking it was supposed to be a formality. It was not.

**The ratchet had been lying since Phase 0, in three ways.**

1. **It greped `crates/ tests/`, and `tests/` does not exist at the repository root.** Whether that produced a false answer depended on which `grep` was on PATH — GNU grep and ugrep disagree about the exit status when one argument is unreadable and another matched. A gate whose answer depends on the grep implementation is not a gate.

2. **It scraped `crate::name` from the whole §8 table**, catching the *Enforcement* column's function names — `revalidate::last_mile`, `gas::choose_limit`, `sizing::refine_discrete`, `allocation::certify` — and counting four functions as four missing tests. The denominator was 54 where §8 names 50 tests.

3. **It only understood Rust test names.** Thirteen invariants are enforced by `forge` tests, a CI grep, or a process gate, and every one was invisible: they contributed to neither side of the ratio. The gate reported a fraction of a fraction and called it coverage.

The rewritten gate recognises four kinds of enforcement — Rust test, Solidity test, CI script, process gate — and reports anything it cannot check as **UNCHECKABLE** rather than passing over it. An invariant whose enforcement the coverage gate cannot verify is exactly the thing such a gate must not hide.

**Honest coverage went 17/46 → 36/46 over this task**, without weakening anything:

| Step | Coverage |
|---|---|
| The broken gate's claim | "24/54" — a number about nothing |
| First honest measurement | **17/46** |
| Phase 5–7 tests renamed to their declared names | 27/46 |
| §8's names reconciled with the implemented ones | 36/46 |

**`scripts/ci/secret_scan.sh` had never existed.** §8 named it as INV-46's enforcement — *"private keys and credentials never enter logs, metrics or telemetry"* — and `.github/workflows/ci.yml` carried a comment explaining why it was "NOT wired yet, red on arrival, but only on test fixtures". Both described a file with no commit in any branch. It was found only because the rewritten ratchet checks artefacts other than Rust tests.

It is written and wired now, and it is deliberately strict:

- **No blanket test allowance.** "Tests may contain secrets" is how a real key hides in a fixture. A finding is suppressed only by a `secret-scan:allow <why>` marker **within five lines of the value**, so the claim is made where the value is; the suppression count is printed on every run so they cannot accumulate invisibly. There are two, both published anvil test keys, each justified at its site.
- **Detection is windowed over three lines, not one.** A per-line grep sees `let private_key =` and `"0x<64 hex>"` on separate lines and objects to neither — which is the shape the real mistake takes. The window found two hits a per-line version missed.
- Five leak shapes are mutation-tested: a key inline, a key split across lines, a real-looking provider key in a URL path, a secret reached past its wrapper inside a log call, and a tracked credential file. A codehash on a `B256` line is checked *not* to be flagged, and a suppression marker seven lines away is checked *not* to suppress.
- `lib/` and `docs/` are excluded as vendored third-party trees, and the exclusion pattern is itself asserted to match only those two, so a third cannot be added quietly.

**Two invariants remain unenforceable as written**, recorded rather than closed:

- **INV-16** named "CI grep: no quoter call reachable from `apex-econ`" with no script. Now named `scripts/ci/no_quoter_in_econ.sh` and marked *not yet written*.
- **INV-45** is a process gate in `docs/apex/GATES.md`, which does not exist. The only invariant the coverage gate reports as uncheckable.

**The naming policy, stated once rather than argued per test.** §8's name is a specification of *what must be proven*; the implementation's name is often a better statement of *what is proven*. Where §8's name was at least as precise the test was renamed to it; where the implemented name said more, §8 was amended to it. Either way the register and the code now agree, which is the only property that matters to a reader trying to find the test that enforces an invariant.
...
| BP-002 | Flashblocks as a speculative branch/overlay engine, not a scalar pending state (§0.2, §5) | 1, 7 | `apex-state::branch` | `crates/apex-state/src/branch.rs` | `state::a_divergent_branch_is_rolled_back_and_names_its_candidates` | G-STATE-2 |
...
| BP-040 | **No global mutable state as shared truth**; workers get immutable snapshots (§5.3) | 1 | `apex-state::versioned` | `src/versioned.rs` | `state::a_reader_keeps_its_snapshot_across_a_write` + CI grep | G-STATE-1 |
| BP-041 | Rollback/divergence handling with the 4 named measurements (§5.4) | 1 | `apex-state::branch` | `src/branch.rs` | `state::a_divergent_branch_is_rolled_back_and_names_its_candidates` | G-STATE-2 |
...
| BP-043 | Feed integrity: 10 tracked counters; gap → UNSAFE → stop → backfill → verify → resume (§5.6) | 1 | `apex-state::feed::integrity` | `src/feed/integrity.rs` | `state::a_sequence_gap_marks_the_branch_unsafe` | G-STATE-1 |
| BP-044 | Two transports where available; disagreement never resolved by vote; only VERIFIED authorizes a ticket (§5.6) | 1 | `apex-state::feed::arbiter` | `src/feed/arbiter.rs` | `state::contradicting_feeds_are_never_resolved_by_majority_vote`, `state::unsafe_state_is_unconstructible_as_verified` | G-STATE-1 |
