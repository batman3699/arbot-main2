...
| **B-13** | **`forge test` is not green, and the plan's Phase 0 gate would have baked that in.** Measured 2026-09-22: `testEnvAddressLookupAcceptsEthereumLongPrefixAlias()` fails because `_defaultUniV3Router(1)` returns the hard-coded mainnet router `0x68b3…Fc45` **instead of** the env-configured address — a chain-1 default silently overriding explicit configuration. `testDeployTransfersRouterOwnershipToConfiguredExecutorOwner()` also fails: the router owner is left as the deployer, not the configured executor owner. `Deploy.s.sol` has four such `chainId == 1` hard-coded fallbacks (`:346` Balancer vault, `:354` Aave pool, `:362` Permit2, `:370` UniV3 router). A silent chain-1 default is the same class of fault as C-11's fabricated addresses: config that looks applied and is not. | **High** | `forge test` output; `script/Deploy.s.sol:346,354,362,370` |
...
| `script/Deploy.s.sol` | Foundry deploy script | **Two failing tests + four silent chain-1 fallbacks** (B-13) | §31 deployment; §26.2 executor address/version validation | **ADAPT** | Not "working". The chain-1 hard-coded defaults override explicit env configuration, which is C-11's failure class in a different place. Fixed in Phase 0 Task 0.2a, before the baseline is frozen. | — | `script/` |
...
|---|---|---|---|
| `contracts/executor/BatchRouter.sol` | No test references it; no production call site found | Trace all callers; if none, REMOVE | Phase 5 |
...
| **INV-08** | `critical_state_gap_used_for_trade == 0` (§5.6, §29.6, gate 33) | A ticket may be admitted only from a state with `ReconstructionStatus::Verified`; the type system enforces it (`VerifiedState` newtype is the only admission input) | `state::gap_blocks_ticket_admission`; `state::unsafe_state_is_unconstructible_as_verified` (compile-fail test) | counter | Stop dependent tickets, backfill, verify, resume | G-STATE-1 |
| **INV-09** | No FIFO on the capture path; an authorized live ticket is never preempted (§29.4, §57.1.2) | Priority scheduler with the §57.1.2 preemption order; `Authorized` and later statuses are exempt from preemption | `capture::authorized_ticket_never_preempted` (property under synthetic overload) | `capture_path_utilization`, `signer_lane_utilization` | Shed research → slow search → low-confidence sims → low-EV | G-CAP-2 |
...
| **INV-18** | The final size is an exact integer/wei candidate verified by exact AMM evaluation; a continuous optimum is never an execution dependency (§14.3) | `sizing::refine_discrete` is mandatory and returns `DiscreteSize`, the only type `Candidate::input_amount` accepts | `econ::continuous_result_cannot_become_input` (compile-fail); `econ::discrete_refinement_never_worse_than_continuous` (property) | `continuous_size` vs `final_discrete_size` | — | G-ECON-1 |
| **INV-19** | Gas limit (scheduling) and gas used (cost) are never conflated (§23.4) | Separate fields with distinct types: `GasLimit(u64)` and `GasUsed(u64)`; no `From` conversion between them | `econ::gas_limit_is_not_gas_used` (compile-fail) | both exported | Build fails | G-ECON-1 |
| **INV-20** | USD marks may never admit a trade on their own; execution validity holds in token units under the conservative bound (§2.10, gate 32) | EV gate takes token-denominated profit; USD enters only `PnlAttribution` and resource ranking, always as `(low, high)` | `econ::usd_mark_cannot_flip_admission` (property: perturb USD ±50%, admission set unchanged) | `net_profit_usd` bounds | — | G-ECON-1 |
...
| ID | Invariant | Implementation | Test | Monitor | Failure response | Gate |
|---|---|---|---|---|---|---|
| **INV-40** | Every economically attractive but unexecuted candidate receives a machine-readable reason code (§33, gate 36) | `MissLedger::record` is called on every rejection path; `MissReason` has no catch-all | `obs::every_rejection_path_records_a_miss` (exhaustive match test) | miss counts by reason | — | G-OBS-1 |
...
| **INV-42** | A risk trigger produces a graduated response, never an unhandled continue (§28.1) | `RiskPosture` ladder with an exhaustive transition table | `risk::every_trigger_maps_to_a_posture` (table over all §28 triggers) | `risk_posture` gauge | ladder | G-RISK-1 |
| **INV-43** | Every loss is classified into one of the §28.2 classes | `LossClass` enum, exhaustive | `risk::every_loss_is_classified` | loss counts by class | Class over-frequency tightens its gate | G-RISK-1 |
...
| **INV-46** | Private keys and credentials never enter logs, metrics or telemetry (§43) | `Secret<T>` newtype whose `Debug`/`Display`/`Serialize` redact; the `sim_rpc_url` credential-carrying field keeps its existing "keep out of logs" discipline | `sec::secret_never_serializes`; `scripts/ci/secret_scan.sh` (existing) in CI | — | Build fails | G-SEC-1 |
...
- [ ] **Step 2: Diagnose.** The first is a precedence bug: `_defaultUniV3Router(block.chainid)` returns the hard-coded mainnet address even when the env supplies one. Explicit configuration must always win over a built-in default — a default that overrides config is indistinguishable from a fabricated address (C-11) at the point of use.
- [ ] **Step 3: Fix** the precedence in `script/Deploy.s.sol` (env first, default only when unset) and the router-ownership transfer. Do **not** delete the chain-1 defaults yet — that is Phase 15's per-chain profile work; just stop them shadowing config.
- [ ] **Step 4: Run and observe** `forge test` fully green.
- [ ] **Step 5: Commit** (named paths only), then re-run Task 0.1 so the frozen baseline is a green one.
...
3. `forge test` is **fully green** — not merely "matching baseline" (Task 0.2a fixes B-13's two failures first, then Task 0.1 re-freezes).
...
| BP-131 | Loss classification into 9 classes; over-frequency tightens the gate (§28.2) | 6 | `apex-risk::loss` | `src/loss.rs` | `risk::every_loss_is_classified` | G-RISK-1 |
