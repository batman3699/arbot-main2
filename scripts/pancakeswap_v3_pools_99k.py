...
NETWORK = "base"
OUTPUT_FILE = "pancakeswap_v3_pools_99k.jsonl"

# GeckoTerminal DEX IDs for PancakeSwap V3
GECKO_DEXES = [
    "pancakeswap-v3-base",
    "pancakeswap-v3",
    "pancake-v3",
...
HEADERS = {
    ...
    "User-Agent": "BasePancakeV3Scanner/1.0"
}
...
def fetch_dexscreener(query: str) -> List[Dict]:
    ...
    return [
        ...
        and (
            "pancake" in str(p.get("dexId", "")).lower()
            or "pcs" in str(p.get("dexId", "")).lower()
        )
...
def parse_gecko(pool: Dict) -> Optional[Dict[str, Any]]:
    try:
        ...
        if "pancake" not in dex_id and "pcs" not in dex_id:
            return None
...
def main():
    print(f"Scanning PancakeSwap V3 pools on Base ≥ ${MIN_LIQUIDITY_USD:,}")
    ...
    print("\n" + "=" * 70)
    print(f"Found {len(results)} PancakeSwap V3 pools ≥ ${MIN_LIQUIDITY_USD:,}")
    print("=" * 70)
