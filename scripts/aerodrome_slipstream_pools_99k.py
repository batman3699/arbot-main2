#!/usr/bin/env python3
"""
Fetch ALL Aerodrome Slipstream pools on Base with ≥ $99k liquidity
and save them in the same JSONL format as the attached example.

Sources: GeckoTerminal + DexScreener
Output:  aerodrome_slipstream_pools_99k.jsonl
"""
...
NETWORK = "base"
OUTPUT_FILE = "aerodrome_slipstream_pools_99k.jsonl"

# GeckoTerminal DEX IDs for Slipstream
GECKO_DEXES = [
    "aerodrome-slipstream",
    "aerodrome-slipstream-2",
]

# DexScreener search queries that surface Slipstream pools
DEXSCREENER_QUERIES = [
...
HEADERS = {
    ...
    "User-Agent": "BaseSlipstreamScanner/1.0"
}
...
def parse_gecko(pool: Dict) -> Optional[Dict[str, Any]]:
    try:
        ...
        # Keep only Slipstream / Aerodrome CL pools
        ...
        # Fee / tick-spacing extraction (best effort)
        fee = 0
        name = attrs.get("name", "")
        # Gecko sometimes puts fee in attributes or name (CL1, CL100, etc.)
        if "fee" in attrs:
            try:
                raw = float(attrs["fee"])
                # if it's already in pips-like form keep it, otherwise convert
...
def main():
    print(f"Scanning Aerodrome Slipstream pools on Base ≥ ${MIN_LIQUIDITY_USD:,}")
    ...
    print("\n" + "=" * 70)
    print(f"Found {len(results)} Aerodrome Slipstream pools ≥ ${MIN_LIQUIDITY_USD:,}")
    print("=" * 70)
