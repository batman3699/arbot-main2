//! Fetching concentrated-liquidity pool state from a node.
//!
//! The other half of `arb-exec`'s `cl_sim`. What it computes moved to
//! `apex_math::cl_state`; what it *reads* is here, because reading needs a
//! provider and `apex-math` may not name one.
//!
//! The five environment gates (`ARBOT_LOCAL_CL_QUOTES`, `ARBOT_CL_MULTI_TICK`,
//! `ARBOT_CL_LADDER_WORDS`, `ARBOT_CL_MAX_TICKS`, `ARBOT_CL_QUOTE_PARITY`)
//! deliberately did NOT come along. They are read at fifteen call sites across
//! `venues`, `plan`, `sizing` and `cl_parity_gate`, so converting them to
//! injected configuration is a refactor of the live pricing path rather than a
//! file move -- it belongs to Task 2.6, which dismantles that env plane as its
//! actual subject. Until then they stay in the legacy crate and this module
//! takes the values it needs as arguments.

use anyhow::{Context, Result};
use apex_math::cl_state::{quote_exact_input_single_tick, ClPoolState};
use ethers::{
    prelude::*,
...
where
    C: JsonRpcClient + Clone + Send + Sync + 'static,
{
    // The `ARBOT_CL_QUOTE_PARITY` check that used to sit here is gone. Gating
    // is the CALLER's job: a function named `log_cl_quote_parity` that silently
    // does nothing because of a process-global is worse than one that does what
    // its name says, and this crate does not read the environment.
    //
    // Recorded while moving it: this function has NO callers, and neither did
    // the flag. It is the local-vs-quoter comparison Task 2.2's three-way
    // differential needs, which is the reason it is being carried forward
    // rather than deleted.
